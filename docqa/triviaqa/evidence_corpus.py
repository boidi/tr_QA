...
def build_tokenized_files(filenames, input_root, output_root, tokenizer, override=True) -> Set[str]:
    ...
    for filename in filenames:
        ...
        if not override and exists(out_file):
            continue
        with open(join(input_root, filename), encoding="utf-8",errors='ignore') as in_file:
        ...
        with open(join(output_root, out_file), "w", encoding="utf-8",errors='ignore') as in_file:
...
def build_tokenized_corpus(input_root, tokenizer, output_dir, skip_dirs=False,
    ...
    with open(voc_file, "w",encoding="utf-8",errors='ignore' ) as f:
...
class TriviaQaEvidenceCorpusTxt(object):
    ...
    def get_vocab(self):
        with open(join(self.directory, "vocab.txt"), "r",encoding ="utf-8",errors='ignore') as f:
